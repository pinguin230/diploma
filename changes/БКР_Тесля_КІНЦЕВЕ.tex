...
\protect\phantomsection\label{_Toc230130247}{}АНОТАЦІЯ
...
\protect\phantomsection\label{_Toc230130248}{}ABSTRACT
...
\hyperref[_Toc230130247]{АНОТАЦІЯ \hyperref[_Toc230130247]{5}}

\hyperref[_Toc230130248]{ABSTRACT \hyperref[_Toc230130248]{7}}

\hyperref[_Toc230130249]{ПЕРЕЛІК УМОВНИХ СКОРОЧЕНЬ, СИМВОЛІВ І ТЕРМІНІВ
\hyperref[_Toc230130249]{11}}

\hyperref[_Toc230130250]{ВСТУП \hyperref[_Toc230130250]{13}}

\hyperref[_Toc230130251]{РОЗДІЛ 1. АНАЛІЗ ПРЕДМЕТНОЇ ОБЛАСТІ ТА
ПОСТАНОВКА ЗАДАЧІ \hyperref[_Toc230130251]{16}}
...
\protect\phantomsection\label{_Toc230130250}{}ВСТУП
...
\protect\phantomsection\label{_Toc230130251}{}РОЗДІЛ 1. АНАЛІЗ
...
вирішуватися евристично або методами машинного навчання~{[}11{]}. SPIRAL
...
розміщення ядра симуляції розміщується на сервері, що вносить мережеву
затримку, несумісно з вимогою NF2 (час відгуку \(\leq 100\)~мс), і
потребує підтримки серверної інфраструктури.
...
\item
  ...
  (стадія 1);
\item
  16~вузлів \texttt{twiddle} (tw-\(r\)-\(k_{1}\)) -- множать на
  поворотні множники (стадія 2);
\item
...
\caption{Таблиця 2.7.\\
Поля типу \protect\texttt{NodeSpec}}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Поле
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
Алгоритм~оперує чотирма матрицями даних розміром \(4 \times 4\):
...
\item
  ...
  активувалися, відображаються яскравішими.
\end{itemize}
...
Рис.2.1. Макет інтерфейсу веб-симулятора потокового 16-точкового ШПФ:
центральна зона GraphView, верхня панель Controls, права панель
SpectrumView + Metrics, нижня панель TerminalOutput. Джерело: розроблено
автором.
...
Менеджер пакетів -- \textbf{pnpm}: файл \texttt{pnpm-lock.yaml} гарантує
...
\caption{Рис. 3.1. Структура директорій проєкту. Джерело: розроблено
...
тестування полегшує пошук місць для внесення змін.
...
якщо виконана, токен видаляється з черги і поміщається у \texttt{inbox}
відповідного вузла-приймача, емітується подія \texttt{onToken}. По-друге
перевірка готовності: вузол є \emph{готовим} якщо e \texttt{inbox} є
токен на \emph{кожному} вхідному порті
...
\texttt{NodeSpec.params.twiddle.k}:
...
реалізує шаблон Factory і будує DFG за дев'яться кроками:
...
і поточна координата визначається лінійною інтерполяцією між центрами
вузлів-кінців:
...
\texttt{Infinity}\texttt{)}; потіи \texttt{sampleQueues}\texttt{()} і
...
\texttt{src}\texttt{/}\texttt{workers}\texttt{/}\texttt{ttf.worker.ts}\texttt{\ }через
...
у sim- Store, а компоненти переведено на селективнi пiдписки. Для
коректної демонстрацiї алгоритму необхідно було вiдокремити логiчний час
симуляцiї вiд астрономiчного часу браузера; це вирiшено введенням
змiнної simTime i множника speed. Нарешті, для збереження вiдтворюваості
...
а важкі розрахунки а важкі розрахунки координат токенів на ребрах
виконуються лише для видимої області через
\texttt{ReactFlow.viewportBounds}. Середня тривалість одкого кроку
...
\texttt{-\/-}\texttt{noEmit}; job \texttt{test} запускає Jest з
...
\texttt{pnpm}\texttt{\ }\texttt{install}\texttt{\ \&\&\ }\texttt{pnpm}\texttt{\ }\texttt{run}\texttt{\ }\texttt{build}
...
\texttt{jsdom} для UI-тестів; трансформатор \texttt{ts-jest} з
\texttt{tsconfig.test.json}; поріг покриття -- не менше 70~\% тверджень,
...
\texttt{pnpm}\texttt{\ }\texttt{test}\texttt{\ -\/-}\texttt{coverage},
становить:
...
лінтера та тестів. Проведено модульне тестування у Jest з покриттям
...
\(\varepsilon = 10^{- 9}\), \emph{кількісно оцінити} виграш у
обчислювальній складності порівняно з прямим ДПФ, \emph{виміряти}
паралелізм dataflow-моделі та зіставити його з теоретичним коефіцієнтом
а також \emph{підтвердити нефункціональні вимоги} NF1--NF8.

Очікувані результати: коректний спектр для усіх тестових сценаріїв;
максимальна абсолютна похибка \(\lesssim 10^{- 12}\); коефіцієнт
прискорення за множеннями \(K_{M} \approx 28 \times\); коефіцієнт
паралелізму \(K_{p} \approx 2,9\) за вузлами; частота кадрів анімації
...
Eрмітова симетрія & \(|X(N - k) - X^{*}(k)|\) & \(< 10^{- 12}\) для
\(x\mathbb{\in R}\) \\
...
\def\labelenumi{\arabic{enumi}.}
\item
  Верифікація \textbf{--} запуск шести тестових сценаріїв з порівнянням
  результатів із еталоном.
\item
  Точність -- вимірювання абсолютної і відносної похибки у критичних
  точках спектра.
\item
  Складність -- підрахунок фактичних операцій за інструментованою
  версією ядра симуляції.
\item
  Паралелізм - вимірювання \texttt{latencyEmaMs} і
  \texttt{throughputFps} у стаціонарному режимі та побудова часової
  діаграми активацій.
\end{enumerate}

Часові характеристики наведено в умовних одиницях симуляційного часу, що
ізолює структурні властивості DFG від варіативної продуктивності
конкретного браузера. Експерименти проведено на машині з процесором
Intel Core i5 11-го покоління, 16\textasciitilde ГБ ОП, ОС
Windows\textasciitilde11 та браузером Chrome\textasciitilde120.
...
\texttt{PresetManage}\texttt{к}. Короткий опис сценаріїв наведено у
табл. 4.2. Застосований трирівневий підхід: \emph{модульне тестування}
функцій (\texttt{cplxAdd}, \texttt{cplxMul}, \texttt{computeDFT4},
\texttt{makePreset}) у Jest~29; \emph{інтеграційне тестування} повного
конвеєра \texttt{DataflowRuntime} з мок-подачею токенів;
\emph{функціональне тестування} симуляції на реальному UI через пресети
з перевіркою \texttt{SpectrumView} та
\texttt{compareWithFFT}\texttt{()}. Перевірки продуктивності проведено
за черех вбудовані метрики Zustand-сховища (\texttt{fpsEma},
\texttt{tickTimeMs}) і Chrome DevTools Performance tab.
...
Сценарій вважається надійним, якщо функція
\texttt{compareWithFFT}\texttt{(}\texttt{sinkValues}\texttt{,\ }\texttt{refSpectrum}\texttt{,\ }\texttt{eps}\texttt{)}
повертає порожній об'єкт
\texttt{\{}\texttt{mismatches}\texttt{:\ \{\}\}}. жоден
\texttt{SinkNode} у GraphView не отримує червоне підсвічування, а
значення \texttt{SpectrumView} збігаються з аналітичним еталоном у межах
\(\varepsilon\). Мінімальні вимоги до середовища виконання: Node.js
18.0, pnpm/npm 9.0. Chreme/Edge 100, Firefox 98; оперативна пам'ять 512
МБ, дисковий простір 200 МБ.

\protect\phantomsection\label{_Toc229681167}{}5.3. Результати
експерименту

Найінформативнішим для аналітичної верифікації є сценарій
\texttt{ramp}\texttt{\ }\(x(n) = n\), \(n = 0,\ldots,15\): нульовий
спектральний відлік

\[X(0) = \sum_{n = 0}^{15}n = \frac{15 \cdot 16}{2} = 120.\]

Для \(k \neq 0\) зі суми геометричного ряду, помноженого на~\(n\):
...
Виміряні значення симулятора та еталонні значення бібліотеки
\texttt{fft.js} всіх \(k = 0\ldots 15\). Наведено у таблиці 4.3.
...
Знак «» означає \(|X_{\text{sim}}(k) - X_{\text{ref}}(k)| < 10^{- 9}\).
Різниця між \(Im\) симулятора і еталону не перевищує машинну точність
числа з плаваючою крапкою (\texttt{double}). Для дійсного вхідного
сигналу \(x(n\mathbb{) \in R}\) спектр задовольняє властивість
\(X(N - k) = X^{*}(k),\ \)що підтверджує антисеметрію уявних частин:
...
Зведені результати верифікації по всіх шести сценаріях наведуно у табл.
4.4. Функція \texttt{compareWithFFT}\texttt{()} у всіх шести випадках
повернула порожній об'єкт
\texttt{\{}\texttt{mismatches}\texttt{:\ \{\}\}}; максимальна абсолютна
похибка не перевищила \(3 \times 10^{- 12}\), що значно менше порогу
\(\varepsilon = 10^{- 9}\).
...
\caption{Зведені результати функціональної верифікації}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Сценарій
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
Обчислювальна складність алгоритму \emph{generateDFT4x4} порівняно з\\
прямим ДПФ при \emph{N=16} підрахована за інструментованою версією
ядра\\
...
Критичний шлях \(N = 16\) з урахуванням затримок вузлів та дуг~(Veen
1986):

\[T_{kr} = 600 + 400 + 600 + 200 + 600 + 400 + 600 = 3400\]

умовних одиниць. Послідовний час виконання без паралелізму:
...
\(K_{p}^{\text{теор}} = 6400/2200 \approx 2,9\). Часову діаграму
активацій вузлів при ідеальному паралельному виконанні подано у табл.
4.6.
...
Виміряні показники UI-продуктивності у трьох основних браузерах наведено
у табл 4.7. В всії браузерах pfsEma = 60. tickTimeMs \textless3mc? Heap
\textless{} 55 МБ, що підтверджує виконання NF1, NF3, NF5.
...
\caption{Продуктивність UI у різних браузерах}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Браузер
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
характеристикою класу зростання~(Nussbaumer 1982).
...
на критичному шляху: \(4 \times 600 = 2400\)~одиниць із загальних 3400
...
(EMA з коефіцієнтом \(\alpha = 0,15\)) точно збігається з теоретичним
критичним шляхом: вимірювана наскрізна затримка від \texttt{originT} до
\texttt{sink} становить \(\approx 3400\)~умовних одиниць при
\texttt{speed}~\(= 1\).
...
(\(\varepsilon_{\text{mach}} \approx 2,22 \times 10^{- 16}\) помноженої
на \(\log_{4}N \times N \approx 32\) арифметичних операцій), що
пояснюється накопиченням помилок у тригонометричних функціях
\texttt{Math.cos} і \texttt{Math.sin} при обчислені поворотних множників
Протягом усього циклу тестування критичних розбіжностей не виявлено;
дрібні UI-розбіжності (неправильне округлення у
\texttt{SpectrumView.tsx}\texttt{)} виявлено і виправлено під час
розробки.
...
систематично \\
Відкритий код (MIT) з тестовим покриттям 85\%+ & Обмежений набір
...
NF4 & Точність \(< 10^{- 9}\) & \texttt{compareWithFFT}\texttt{()} на 6
сценаріях & \\
NF5 & Кросбраузерність & Тестування у Chrome, Edge, Firefox & \\
NF6 & Відкритий код (MIT) & Ліцензія проекту & \\
NF7 & Портативність & Збірка
\texttt{pnpm}\texttt{\ }\texttt{run}\texttt{\ }\texttt{build} без
помилок & \\
NF8 & Тестове покриття \(> 80\%\) & Jest coverage звіт & \\
\end{longtable}

\protect\phantomsection\label{_Toc229681182}{}Обраний варіант
клієнтського SPA забезпечує найкращий компроміс між простотою
розгортання та повноцінною інтерактивністю, що підтверджується
порівнянням у табл. 4.9 і обґрунтуванням у підрозд. 2.1.

4.5. Обговорення та практична оцінка системи

Типовий сценарій використання виглядає так: студент або викладач
відкриває застосунок, обирає пресет \texttt{sin1} (один гармонічний тон)
і натискає «Run». Протягом \(\approx 3400\)~умовних одиниць
симуляційного часу (\(\approx 3,4\)~с реального часу при
...
решта нульові -- наочне підтвердження коректності обчислення (UC1, UC3).
Середній час від першого запуску до отримання результату \(\approx 45\)
с додатковий сценарій редагування вхідних відліків доступний без
перезапуску (UC2).
...
(heap залишається в діапазоні 48--52~МБ), зависань UI не виникає.
...
(onToken, onFire, onOutput) з мітками часу; журнал можна скопіювати у
буфер обміну для подальшого аналізу. При модельованому навантаженні
(циклічна зміна пресетів з періодом 5~с, \texttt{speed}~\(= 10\))
...
\texttt{fpsEma} до \(\approx 38\)~fps, що визначає практичну межу
\(200\)~вузлів без віртуалізації.
...
\texttt{return}\texttt{\ ()\ =\textgreater{}\ }\texttt{unsubscribe}\texttt{()}\texttt{\ }у
\emph{useEffect} cleanup. Застосунок придатний для використання в режимі
...
(Sentry/Plausible) та WebWorker-ізоляцію обчислень при N \textgreater{}
64.
...
коректність реалізації: функція \emph{compareWithFFT()} повернула
порожній об\textquotesingle єкт розбіжностей в усіх випадках;
максимальна абсолютна похибка не перевищила 3×10⁻¹², що на три порядки
менше від порогу ε=10⁻⁹ (NF4 виконано).

Аналіз обчислювальної складності показав коефіцієнт прискорення Kₘ≈28,4×
за нетривіальними множеннями порівняно з прямим ДПФ, що відповідає класу
O(N log₄ N) (NF2 виконано).

Аналіз паралелізму dataflow-моделі виявив практичний коефіцієнт Kₚ≈1,88
з урахуванням затримок дуг і теоретичний Kₚᵗᵉᵒʳ≈2,9 лише за вузлами;
паралельна активація чотирьох незалежних dft4-вузлів на кожному ярусі
експериментально підтверджена часовою діаграмою (табл. 4.6).

Продуктивність UI у трьох браузерах стабільно тримається на рівні 60 fps
із часом tick()\textless3 мс (NF1, NF3, NF5 виконано).

Порівняння з альтернативними підходами (табл. 4.9) обґрунтовує вибір
клієнтського SPA як оптимального компромісу між простотою розгортання,
кросбраузерністю та інтерактивністю.

Усі вісім нефункціональних вимог NF1--NF8 підтверджено (табл. 4.10);
симулятор готовий до застосування як навчально-дослідницький інструмент
для вивчення потокового виконання алгоритмів ЦОС.

Обмеження дослідження: фіксований розмір N=16; систематичне тестування у
Safari і на мобільних пристроях не проведено; WebWorker-ізоляція не
реалізована.

Перспективні напрями: масштабування на N∈\{64,256,1024\}, перенесення
ядра у WebWorker, інтеграція з LMS-платформами через LTI-протокол,
апаратна верифікація DFG-специфікації на FPGA.
...
\item
  ...
  \texttt{tick}\texttt{()}~(\hyperref[eq:delivery]{{[}eq:delivery{]}})--(\hyperref[eq:emit_time]{{[}eq:emit\_time{]}}).
...
\item
  ...
  спектрального відліку; максимальна абсолютна похибка не перевищила
