...
\protect\phantomsection\label{_Toc229682904}{}РОЗДІЛ 1. АНАЛІЗ
ПРЕДМЕТНОЇ ОБЛАСТІ ТА ПОСТАНОВКА ЗАДАЧІ

\begin{quote}
\protect\phantomsection\label{_Toc229682905}{}1.1. Характеристика
предметної області
\end{quote}

1.1.1. Контекст застосування
...
частотний простір~{[}1{]}{[}2{]}. За обсягом щоденно виконуваних
операцій ШПФ залишається однією з найзавантаженіших обчислювальних
процедур у світі.
...
1.1.2. Опис проблеми та її актуальність
...
прогалина між трьома рівнями опису алгоритму:

\begin{itemize}
\item
  \emph{математична модель} --- матричні факторизації, формули метеликів
  і поворотних множників, що подаються у підручниках із ЦОС~(Nussbaumer
  1982; Burrus and Parks 1985);
\item
  \emph{формальна потокова модель} --- DFG і Synchronous Dataflow (SDF)
  Лі та Месершмітта, що описують вузли, дуги, токени та правило
  активації~{[}1{]}{[}4{]};
\item
  \emph{виробнича реалізація} --- бібліотеки FFTW, ARM~CMSIS-DSP,
  NVIDIA~CuFFT, SPIRAL, Intel~IPP, що оптимізовані виключно під
  максимальну продуктивність на конкретних
...
затримки та паралельних шляхів у графі. Студент, викладач або
інженер-початківець, що вивчає архітектуру потокового обчислювача ШПФ,
не має інструменту, який одночасно є математично достовірним (виконує
реальний алгоритм), формально обґрунтованим (відповідає SDF-моделі) і
візуально інтерактивним (показує рух токенів у реальному часі).
...
\begin{quote}
\protect\phantomsection\label{_Toc229682906}{}1.2. Огляд сучасних
підходів та рішень
\end{quote}
...
напрямки та проводиться їх порівняльний аналіз.

1.2.1. Програмні бібліотеки обчислення ШПФ
...
потоків (warps/wavefronts)~{[}10{]}. Для подолання цього застосовується
автотюнінг: А.~Нукада та С.~Мацуока~показали, що адаптивний вибір
відступів у спільній пам'яті та кількості активних блоків потоків
дозволяє перевершити навіть вендорну бібліотеку NVIDIA~CuFFT {[}14{]}.
...
1.2.2. Методи візуального моделювання обчислювальних систем
...
виконувані симулятори з числовою верифікацією.

\textbf{Загальні нотації моделювання} --- UML~Activity, IDEF0/IDEF3, DFD
Йордана/ДеМарко --- орієнтовані на control-flow
парадигму~{[}15{]}{[}16{]}. Вони не мають формальної семантики токенів і
не забезпечують однозначного відображення паралелізму за даними, тому
для потокового обчислювача ШПФ породжують надлишкові конструкції
\texttt{fork}/\texttt{join}.

\textbf{DFG/SDF} (Data Flow Graph і Synchronous Dataflow) ---
спеціалізовані моделі, розроблені для задач ЦОС~{[}7{]}{[}17{]}{[}18{]}.
У SDF-моделі кількість токенів, що споживаються та виробляються кожним
вузлом за одну активацію, є фіксованою і відомою на етапі статичного
аналізу. Це дозволяє виконувати статичне планування без ризику взаємних
блокувань, гарантувати детермінованість результатів і перевіряти
консистентність графа алгебраїчно через топологічну матрицю~{[}19{]}.
...
С.~С.~Буррус формалізує зв'язок між матричною факторизацією та графом
алгоритму через рівняння
\(\mathbf{X}^{\mathbf{'}} = \mathbf{C} \cdot \mathbf{D} \cdot \mathbf{A} \cdot \mathbf{X}\),
де кожна матриця відповідає окремому ярусу графа~(Burrus and Parks
1985). Ці діаграми є статичними і не моделюють динаміку виконання. У
вітчизняній науковій традиції аналогічний підхід застосовувався у
роботах ОКБ~ЕІВТ (М.~Розенблат, 1970-ті~рр.) та у монографії
М.~М.~Яцимірського (1997~р.). Зокрема, у Фізико-механічному інституті
ім.~Г.~В.~Карпенка НАН~України в 1980-х роках було розроблено векторний
спецпроцесор ШПФ на основі саме dataflow-парадигми з блоком арифметики,
що апаратно реалізовував чотири базові потокові операції~{[}2{]}.

1.2.3. Ступінь дослідженості проблеми та вузькі місця

Узагальнення розглянутих джерел дозволяє зафіксувати такі прогалини.
По-перше, виробничі бібліотеки (FFTW, CMSIS-DSP, CuFFT, SPIRAL,
Intel~IPP) оптимізовані під продуктивність і не надають жодних
...
Таким чином, більшість авторів вирішує задачу обчислення ШПФ або суто
продуктивнісним підходом (FFTW, CuFFT, SPIRAL), або суто теоретичним
(DFG/SDF-моделі, сигнальні графи), однак підхід, який поєднує формальну
SDF-модель, виконуваний алгоритм із числовою верифікацією та
інтерактивну веб-візуалізацію, у літературі не представлений.

1.2.4. Порівняння існуючих рішень

Порівняльна характеристика розглянутих рішень за ключовими
характеристиками (табл. 1.1).
...
\begin{quote}
\protect\phantomsection\label{_Toc229682907}{}1.3. Системний аналіз
задачі
\end{quote}
...
Потенційні користувачі та бенефіціари розроблюваного симулятора (табл.
1.2)
...
DFG зі спостереженням токенів, метрик і наскрізної затримки.
...
1.4.1. Характер даних, що опрацьовуються
...
референтні реалізації мають бути належно процитовані, а отримані
...
\def\labelenumi{\arabic{enumi}.}
\item
  обчислює дискретне перетворення Фур'є
\end{enumerate}
...
(табл.~1.4) у відповідності до ISO/IEC~25010.
...
1.5.5. Діаграма прецедентів (Use Case)
...
Рис. 1.1. Use Case-дiаграма симулятора потокового обчислювача ШПФ
...
Математичне та алгоритмічне обґрунтування (виведення формул radix-4
\(4 \times 4\), матричні факторизації, формалізація DFG/SDF) виноситься
до розділу~2, де воно подається у складі проєктного обґрунтування
обраних методів і архітектури системи.
