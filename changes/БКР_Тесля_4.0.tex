...
2 & РОЗДІЛ 1. Аналіз предметної області обчислення швидкого
перетворення Фур'є під керуванням потоку даних, огляд існуючих рішень,
системний аналіз задачі та постановка задачі & 15.02.26 -- 29.03.26 & \\
...
структури даних та інтерфейс користувача & 04.04.26 -- 26.04.26 & \\
4 & РОЗДІЛ 3. Реалізація програмного рішення: ядро
\passthrough{\lstinline!DataflowRuntime!}, обчислювальні вузли, побудова
DFG 16-точкового ШПФ, тестування та розгортання & 12.04.26 -- 17.05.26 & \\
...
підтвердження вимог & 17.05.26 -- 25.05.26 & \\
6 & & & \\
7 & & & \\
\end{longtable}
}
...
відеопотоків. За оцінками IEEE Signal Processing Society, ШПФ
...
максимальний паралелізм без явних механізмів синхронізації {[}19{]}. Граф
...
відповідає окремому ярусу графа {[}26{]}. Ці діаграми є
...
пiдготовки вхiдних даних. Деталізований опис прецедентів наведено у
табл.~\ref{tab:usecases}.
...
(Use Cases) симулятора}\label{tab:usecases}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{ID}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
Загальна контекстна діаграма системи наведена на рис.~\ref{fig:c4-context}.

\begin{figure}[H]
\centering
\includegraphics[width=4.952in,height=4.79265in]{media/media/image3.png}
\caption{C4 Context-діаграма веб-симулятора потокового обчислювача ШПФ.}
...
прискорення у \(\approx 28\) разів за кількістю множень {[}23{]}{[}26{]}.
...
Архітектура симулятора описується на двох рівнях моделі C4 {[}4{]}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!source!} & 16 & 0 & 1 & -- & Генерація \(x(n)\) \\
\passthrough{\lstinline!dft4!} & 8 & 4 & 4 & 400 & 4-точкове ДПФ (метелик з
підрозд. 2.2) \\
...
Поля типу \passthrough{\lstinline!Edge!} наведено у табл.~\ref{tab:edge-fields}.
...
\caption[Поля типу Edge]{Поля типу \passthrough{\lstinline!Edge!}}\label{tab:edge-fields}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Поле
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
{[}7{]}{[}29{]}. Він відповідає за виконання
...
{[}7{]}{[}29{]}. Алгоритм складається з чотирьох
...
характеристикою класу зростання {[}23{]}. У реальній топології
...
\item
  ...
  обрано клієнтський SPA на React Flow як засіб, що поєднує достовірність
  обчислення з доступністю без встановлення (підрозд. 1.1--1.3).
\item
  ...
  множень і таблицею поворотних множників \(W_{16}^{rk_{1}}\)
  (підрозд. 2.2).
\item
  Побудовано граф потоку даних обчислювача (56 вузлів чотирьох типів,
  64 дуги з \passthrough{\lstinline!delay!} \(= 600\)) з критичним шляхом
...
\item
  ...
  React Flow 12 / Zustand 4 з ядром
  \passthrough{\lstinline!DataflowRuntime!} і чотирикроковим методом
  \passthrough{\lstinline!tick()!} (доставка \(\rightarrow\) перевірка
  готовності \(\rightarrow\) активація \(\rightarrow\) розміщення
  вихідних токенів); функція
...
\item
  ...
  \passthrough{\lstinline!compareWithFFT()!} підтвердила бітовий збіг з
  еталоном \passthrough{\lstinline!fft.js!} (максимальна абсолютна
  похибка \(= 0\), з запасом задовольняє поріг
...
\item
  ...
  встановлення, зручність): чотири з них (NF1, NF5, NF6, NF7)
  перевірено автоматизованим харнесом, решта -- ручними замірами у
  Chrome, Edge, Firefox (табл.~4.10).
\end{enumerate}
...
спеціальності 122 «Комп'ютерні науки»: виконує реальний алгоритм
radix-4 (а не імітацію), доступний через будь-який сучасний браузер без
...
\def\labelenumi{\arabic{enumi})}
\item
  Arm~Ltd. CMSIS-DSP: Complex FFT Functions. Version 1.16.2. URL:~\url{https://arm-software.github.io/CMSIS-DSP/v1.16.2/group__ComplexFFT.html}
...
\item
  ...
  1996. 189~p. URL:~\url{https://ptolemy.berkeley.edu/publications/papers/96/softSynthBook/}.
\item
...
\item
  ...
  2024. URL:~\url{https://www.intel.com/content/www/us/en/docs/ipp/developer-guide-reference/2021-12/fast-fourier-transform-functions.html}
  (дата звернення: 21.05.2026).
\item
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!source!} & 16 & 0 & 1 & 0 & Подача відліку \(x(n)\) у
граф \\
\passthrough{\lstinline!row-dft4!} & 4 & 4 & 4 & 400 & Рядкове 4-точкове ДПФ
(стадія 1) \\
\passthrough{\lstinline!twiddle!} & 16 & 1 & 1 & 200 & Множення на
\(W_{16}^{n_{1}k_{2}}\) (стадія 2) \\
\passthrough{\lstinline!col-dft4!} & 4 & 4 & 4 & 400 & Стовпцеве 4-точкове ДПФ
(стадія 3) \\
\passthrough{\lstinline!sink!} & 16 & 1 & 0 & 0 & Збір відліку \(X(k)\) \\
Всього & 56 & --- & --- & --- & --- \\
\end{longtable}
...
\passthrough{\lstinline!row-2!}, \passthrough{\lstinline!row-3!} & 400 \\
...
\passthrough{\lstinline!tw-3-2!}, \passthrough{\lstinline!tw-3-3!} & 200 \\
\passthrough{\lstinline!col-dft4!} (4) &
\passthrough{\lstinline!col-0!}, \passthrough{\lstinline!col-1!},
\passthrough{\lstinline!col-2!}, \passthrough{\lstinline!col-3!} & 400 \\
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!ramp!} & \(1,42 \cdot 10^{-14}\) &
\(5,32 \cdot 10^{-15}\) & виконано \\
\passthrough{\lstinline!impulse!} & \(0,00\) & \(0,00\) & виконано \\
\passthrough{\lstinline!sin1!} & \(2,22 \cdot 10^{-16}\) &
\(1,11 \cdot 10^{-16}\) & виконано \\
\passthrough{\lstinline!sin3!} & \(2,22 \cdot 10^{-16}\) &
\(1,11 \cdot 10^{-16}\) & виконано \\
\passthrough{\lstinline!sin1+3!} & \(4,44 \cdot 10^{-16}\) &
\(2,22 \cdot 10^{-16}\) & виконано \\
\passthrough{\lstinline!two-impulses!} & \(2,22 \cdot 10^{-16}\) &
\(1,11 \cdot 10^{-16}\) & виконано \\
\end{longtable}
...
4 & \passthrough{\lstinline!tw-2-1!} & 2 & \(+ 0,7071 - 0,7071j\) \\
5 & \passthrough{\lstinline!tw-2-2!} & 4 & \(+ 0,0000 - 1,0000j\) \\
6 & \passthrough{\lstinline!tw-2-3!} & 6 & \(- 0,7071 - 0,7071j\) \\
7 & \passthrough{\lstinline!tw-3-1!} & 3 & \(+ 0,3827 - 0,9239j\) \\
8 & \passthrough{\lstinline!tw-3-2!} & 6 & \(- 0,7071 - 0,7071j\) \\
...
\bottomrule\noalign{}
\endlastfoot
1 & Функціональна IDEF0-схема задачі моделювання потокового обчислювача
ШПФ & IDEF0 & рис. 1.1 & PlantUML \\
2 & Use Case-діаграма симулятора потокового обчислювача ШПФ & UML Use Case
& рис. 1.2 & PlantUML \\
3 & C4 Context-діаграма веб-симулятора потокового обчислювача ШПФ & C4
Context & рис. 2.1 & PlantUML \\
4 & Схема тристадійної radix-4 декомпозиції 16-точкового ШПФ &
Алгоритмічна & рис. Г.1 & PlantUML \\
5 & C4 Container-діаграма архітектури веб-симулятора & C4 Container & рис.
2.3 & PlantUML \\
6 & Граф потоку даних обчислювача 16-точкового ШПФ radix-4 \(4 \times 4\)
& DFG & рис. Г.2 & PlantUML \\
7 & Макет інтерфейсу веб-симулятора потокового 16-точкового ШПФ &
Wireframe & рис. 2.5 & PlantUML \\
8 & Життєвий цикл токена у \passthrough{\lstinline!DataflowRuntime!} &
Sequence & рис. Г.3 & PlantUML \\
9 & Часова діаграма активацій вузлів DFG при \(N = 16\) & Timing & рис.
4.1 & PlantUML \\
\end{longtable}
